...
class VirtualObjectManager {
	...
	// MARK: - Resetting objects
     ...
     static let availableObjects: [VirtualObjectDefinition] = {
        guard let jsonURL = Bundle.main.url(forResource: "ObjectList", withExtension: "json")
    ...
    static let availableObjects1: [VirtualObjectDefinition] = {
    ...
    static let availableObjects2: [VirtualObjectDefinition] = {
        guard let jsonURL = Bundle.main.url(forResource: "ObjectList2", withExtension: "json")
            else { fatalError("missing expected VirtualObjects.json in bundle") }
        do {
            let jsonData = try Data(contentsOf: jsonURL)
            return try JSONDecoder().decode([VirtualObjectDefinition].self, from: jsonData)
    ...
    static let availableObjects3: [VirtualObjectDefinition] = {
        guard let jsonURL = Bundle.main.url(forResource: "ObjectList3", withExtension: "json")
            else { fatalError("missing expected VirtualObjects.json in bundle") }
        do {
            let jsonData = try Data(contentsOf: jsonURL)
            return try JSONDecoder().decode([VirtualObjectDefinition].self, from: jsonData)
 ...
 static let availableObjects4: [VirtualObjectDefinition] = {
        guard let jsonURL = Bundle.main.url(forResource: "ObjectList4", withExtension: "json")
            else { fatalError("missing expected VirtualObjects.json in bundle") }
        do {
            let jsonData = try Data(contentsOf: jsonURL)
            return try JSONDecoder().decode([VirtualObjectDefinition].self, from: jsonData)
     ...
     static func shareObjects(index: Int) -> [VirtualObjectDefinition] {

        if (index == 0) {
            return availableObjects
        } else if (index == 1) {
            return availableObjects1
        } else if (index == 2) {
            return availableObjects2
        } else if (index == 3) {
            return availableObjects3
        } else {
            return availableObjects4
        }
    }
	...
	private func unloadVirtualObject(_ object: VirtualObject) {
		ARViewController.serialQueue.async {
			object.unloadModel()
	...
	func worldPositionFromScreenPosition(_ position: CGPoint,
        ...
        let unfilteredFeatureHitTestResults = sceneView.hitTestWithFeatures(position, coneOpeningAngleInDegrees: 90)
...
//    var VirtualObjects: [VirtualObject] {set get}
}
