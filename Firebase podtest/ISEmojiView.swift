...
public class ISEmojiView: UIView, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    private func updateControlLayout() {
        ...
        searchButton.frame = CGRect(x: frame.width - 360, y: frame.height - 40, width: 40, height: 40)
    }
...
//
    static private func pathOfResourceInBundle(filename:String, filetype: String) -> String? {
        if let filePath = Bundle.main.url(forResource: filename, withExtension: filetype) {
            let strpath: String = filePath.path
            return strpath
        }
        return nil
    }
